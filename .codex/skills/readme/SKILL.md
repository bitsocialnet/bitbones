...
1. **Title + one-paragraph overview** — what it is, who it's for, links to the live app/downloads
...
5. **Getting started** — clone, `corepack yarn install`, `yarn start`, expected dev URL (https://bitbones.localhost via portless, or http://localhost:5173 with `PORTLESS=0`); every command copy-pasteable and tested
6. **Architecture overview** — directory map with one-line descriptions, how a view maps to a protocol hook, where local UI state lives, how the peer-to-peer parts work
...
9. **Building and releasing** — per-platform build commands (web, Android, Electron) and the tag-triggered release flow
10. **Troubleshooting** — only real, observed failure modes with fixes; don't invent generic ones
11. **Contributing / License** — link `AGENTS.md`/docs rather than duplicating policy
...
8. **Don't duplicate other repo docs** — link to `AGENTS.md` and the playbooks in `docs/` instead of restating them; duplicated policy drifts
9. **Never name the protocol's former name or the client this was ported from.** The protocol is
   Bitsocial, the low-level library is PKC, and communities are communities.

## Output

Write directly to `README.md` in the project root. After public-facing English content changes in this repo, run `yarn llms:generate` and commit any resulting `public/llms*.txt` changes (see the AGENTS.md task router).
