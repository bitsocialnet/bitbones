...
Use the task's URL and a unique session name. A reachable canonical `https://bitbones.localhost` server may belong to another checkout; confirm compatibility before using it. A delegated browser checker does not manage dev servers. The task owner records and later stops only the server it started.
...
./scripts/pw-session.sh open --wait=60 "$browser_session" "https://bitbones.localhost/#/" --browser=chrome
