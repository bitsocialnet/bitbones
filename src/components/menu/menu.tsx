...
const pages = new Set(['profile', 'settings', 'communities', 'about', 'inbox', 'u']);
...
const Menu = () => {
  ...
  // only the default feeds are built from a client's default community list, so only they can switch it
  const showDefaultListSwitch = !isPage && (!feedName || feedName === 'all');
  ...
  const changeFeedName = (event: ChangeEvent<HTMLSelectElement>) => {
    ...
    if (feedName === 'goToCommunity') {
      return setGoToCommunityModalIsOpen(true);
    }
  ...
  return (
    <div className={styles.menu}>
      <div id='sticky-menu' className={styles.stickyMenu}>
        ...
        <span className={styles.rightMenu}>
          {showDefaultListSwitch ? (
            <>
              <DefaultListSwitch className={styles.menuItem} />{' '}
            </>
          ) : undefined}
          <select onChange={changeFeedName} className={[styles.feedName, styles.menuItem].join(' ')} value={selectedFeedName}>
            ...
            <option value='goToCommunity'>p/</option>
          </select>{' '}
        ...
        <GoToCommunityModal isOpen={goToCommunityModalIsOpen} setIsOpen={setGoToCommunityModalIsOpen} />
