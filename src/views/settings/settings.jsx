...
const BitbonesSettings = () => {
  return (
    <div className={styles.bitbonesSettings}>
...
const AuthorAddress = () => {
  ...
  if (resolvedAddress) {
    if (resolvedAddress === account?.signer?.address) {
      ...
      helpText = `failed: crypto name text record 'bitsocial' set to wrong public key (set to ${resolvedAddress} instead of ${account?.signer?.address})`;
    }
  }
  if (state === 'succeeded') {
    if (!resolvedAddress) {
      helpText = `failed: crypto name text record 'bitsocial' not set`;
    }
  ...
  const save = () => {
    ...
    setAccount({ ...account, author: { ...account?.author, address: inputValue } });
    alert(`saved`);
  };
...
const AccountSettings = () => {
  ...
  const saveAccount = async () => {
    try {
      ...
      // editing pkcOptions requires a page reload
      if (JSON.stringify(oldAccount.pkcOptions) !== JSON.stringify(newAccount.pkcOptions)) {
