  ...
  scan({
    ...
    onRender: (fiber, renders) => {
      for (const render of renders) {
        const name = render.componentName || fiber?.type?.displayName || fiber?.type?.name;
        if (!name) {
          continue;
        }
...
const notReady = async () => ({ error: 'element-source is not ready yet.' });

const elementSourceApi = {
  ready: false,
  error: null,
  resolve: notReady,
...
import('element-source')
  .then(({ formatStack, resolveElementInfo }) => {
    ...
    Object.assign(elementSourceApi, {
      ready: true,
      resolve,
      resolveBySelector: async (selector) => {
      ...
      formatStack: (stack, maxLines = 3) => (Array.isArray(stack) ? formatStack(stack, maxLines) : ''),
    });
  })
  .catch((error) => {
