...
merged_pr_number_for_branch() {
  ...
  if [ -n "$pr_number" ]; then
    ...
    merged_at="$(gh pr view "$pr_number" --repo bitsocialnet/bitbones --json mergedAt --jq 'select(.mergedAt != null) | .mergedAt' 2>/dev/null || true)"
    [ -n "$merged_at" ] || return 0
    echo "$pr_number"
    return 0
  fi

  gh pr list --repo bitsocialnet/bitbones --state merged --head "$branch" --json number --jq '.[0].number // empty' 2>/dev/null || true
}
