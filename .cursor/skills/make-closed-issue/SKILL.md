...
| `documentation` | README, AGENTS.md, `docs/` changes |
...
```bash
git switch -c fix/blank-community-header
```
...
- **Title**: Short, present-tense, describes the **problem** (not the solution). Use backticks for UI elements, code, or literal strings (e.g. Community page header is blank when `` `useCommunity` `` is given an address).
- **Description**: 2-3 sentences about the problem. Use backticks for UI element names, function/code references (`useCommunityIdentifier()`), and literal text strings. Write as if the issue hasn't been fixed yet.

### 6. Create the issue

```bash
gh issue create \
  --repo bitsocialnet/bitbones \
  --title "ISSUE_TITLE" \
...
- **Scope**: area of the codebase (e.g., `reply tools`, `default lists`, `routing`)
...
Issue #NUMBER created, committed, pushed, and linked to a PR into master.
  ...
  PR: PR_URL
  URL: https://github.com/bitsocialnet/bitbones/issues/NUMBER
```
