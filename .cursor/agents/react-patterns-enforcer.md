...
```bash
git diff --name-only HEAD -- '*.jsx' '*.js'
```
...
| Violation | Fix |
|-----------|-----|
| An address string passed to `useFeed`, `useCommunity`, `useCommunityStats`, or `useCommunitiesStates` | Use `useCommunityIdentifier` / `useCommunityIdentifiers` from `src/hooks/use-community-identifier.js`. These hooks take a `CommunityIdentifier` (`{name}` or `{publicKey}`); an address string returns nothing, silently |
...
| `useState` for shared/global state | Move to a zustand store — a small `createStore` in `src/hooks/` (see `use-theme.js`) or colocated with the component that owns it. There is no `src/stores/` directory in this repo |
...
| TypeScript syntax, `.ts`/`.tsx` files, or type annotations | Rewrite as plain `.jsx`/`.js` — this repo is not TypeScript |
...
Scroll-position effects around `react-virtuoso` and the `document.title` effect in `src/app.jsx` are
...
If lint, the build, or react-doctor breaks due to your changes, fix and re-run. Treat react-doctor as
a reviewer of newly introduced diagnostics, not an aggregate score to grind up. There is no
type-check step and no test suite in this repo.
...
- `file.jsx:42` — address string passed to useFeed → routed through useCommunityIdentifiers

### Violations Found (unfixed)
- `file.jsx:100` — description and why it wasn't auto-fixed
